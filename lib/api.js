...
// Cargo do usuário no workspace: 'owner' | 'admin' | 'agent'
export function getRole() {
  const u = getUser();
  if (!u) return "owner";
  if (u.workspace_owner_id) return u.workspace_role || "agent";
  return "owner";
}
...
export async function api(path, { method = "GET", body, auth = true, rawBody = false } = {}) {
  const headers = {};
