...
// Garante que o usuário tem a coluna padrão "Novo Lead"
async function ensureDefaultColumn(userId) {
...
// Listar colunas + leads do pipeline
app.get('/api/crm/columns', requireAuth, async (req, res) => {
  try {
    await ensureDefaultColumn(uid(req));
    ...
    // Busca leads de cada coluna
    const { data: leads } = await supabase
      .from('leads')
      .select('id, name, phone, source, tags, last_interaction_at, created_at, pipeline_column_id, pipeline_position, avatar_url')
      .eq('user_id', uid(req))
      .order('pipeline_position');

    // Leads sem coluna vão para a coluna padrão
    const defaultCol = columns.find(c => c.is_default);
    const result = columns.map(col => ({
      ...col,
      leads: (leads || []).filter(l =>
        l.pipeline_column_id === col.id ||
        (!l.pipeline_column_id && col.is_default)
...
// Renomear coluna
app.patch('/api/crm/columns/:id', requireAuth, async (req, res) => {
  try {
    const { name, color } = req.body;
    const patch = {};
    if (name) patch.name = name.trim();
    if (color) patch.color = color;
    await supabase.from('crm_columns').update(patch)
...
// Deletar coluna (move leads para a coluna padrão)
app.delete('/api/crm/columns/:id', requireAuth, async (req, res) => {
  try {
    const { data: col } = await supabase
      .from('crm_columns').select('is_default')
      .eq('id', req.params.id).eq('user_id', uid(req)).maybeSingle();
    if (col?.is_default) return res.status(400).json({ error: 'Coluna padrão não pode ser deletada' });

    // Move leads para coluna padrão
    const { data: def } = await supabase
      .from('crm_columns').select('id')
      .eq('user_id', uid(req)).eq('is_default', true).maybeSingle();
    if (def) {
      await supabase.from('leads')
        .update({ pipeline_column_id: def.id })
        .eq('pipeline_column_id', req.params.id)
        .eq('user_id', uid(req));
    }

    await supabase.from('crm_columns').delete()
      .eq('id', req.params.id).eq('user_id', uid(req));
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Mover lead para outra coluna (drag & drop)
app.patch('/api/crm/leads/:id/move', requireAuth, async (req, res) => {
  try {
    const { column_id, position } = req.body;
    await supabase.from('leads')
      .update({ pipeline_column_id: column_id, pipeline_position: position ?? 0 })
      .eq('id', req.params.id).eq('user_id', uid(req));
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
